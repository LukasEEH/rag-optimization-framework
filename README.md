...
- the RAG module (baseline and final configuration after the optimization study) in `/rag`
...
:wrench: **Configuration**
 ...
 4. Define a config file for the RAG module (see [example](./rag/baseline/config.example.json))
    - Note: the sources from the example are supported from the Loader defined in the thesis (see [`loader.py`](./rag/baseline/loader.py)). You can add arbitrary data sources as long as you define your own implementation of the  [`base_loader`](./rag/baseline/interfaces/base_loader.py) interface to handle them.
...
- Note: the benchmark initializes the RAG module under test. When this is initialized and cannot find an index in the specified directory (see [config](./rag/baseline/config.example.json)) it creates one automatically.
